...
\#' Forelesning 1 \############################## \#' `Objective`: learn/review basic commands from `dplyr` \#' \#' R makes use of the \# or \#' signs to add comments, \#' so that you and others can understand what the R code \#' is about.
...
\#' `dplyr- Basic data manipulation` \#' browseURL("https://r4ds.had.co.nz/transform.html") \#' `filter()` - Pick observations (rows) by their values\
...
\#' `summarise()` - Collapse many values down to a single summary \#' `rename()` - Rename columns (variables): \#' `group_by()` - Group rows by columns (variables) \#' \########################################################## \############################################################# \########

rm (list = ls())

library(tidyverse) \# Use the gapminder data-set from the gapminder package library(gapminder) data("gapminder")

#Data exploration str(gapminder) #data structure glimpse(gapminder)

# eller \#' for å kommentere
